...
function getN3Status(get, ctx, out) {
  ...
  const td3c = get('vlcc_spot');

  // D49, 18 Sep 2026. The floating-storage bands alone are an ABSORPTION test:
  // they assume high floating storage means the fleet is soaking up unsold
  // crude, which is a glut signal. The current regime is the inverse — storage
  // is low BECAUSE tonnage is consumed by lengthened voyages (Cape routing adds
  // 18-20 days and $11-14m per trip) and stranded by risk, not because the
  // fleet is free. Scored on absorption alone this node printed green on
  // 18 Sep with TD3C at a record $1.099m/day and 409 vessels stranded.
  //
  // Caveat carried into the bands: TD3C is index-named Ras Tanura-Ningbo but
  // has been Yanbu-referenced since Feb 2026 and runs roughly $5/bbl rich
  // against cleared freight, so these levels are deliberately not tight.
  if (td3c !== null && td3c > 500000) {
    return reason(out, 'red', `TD3C $${Math.round(td3c / 1000)}K/day — freight at crisis levels`);
  }
  ...
  if (td3c !== null && td3c > 250000) {
    return reason(out, 'amber', `TD3C $${Math.round(td3c / 1000)}K/day — freight elevated`);
  }
  return null;
}
