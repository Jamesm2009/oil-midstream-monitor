...
function getN4Status(data) {
  ...
  // Cushing is in K bbl (thousand barrels) — 18M bbl = 18000, 22M bbl = 22000
...
function getN2Status(data) {
  ...
  // Hormuz outbound is the primary signal
  if (outbound && outbound < 15) return 'red';
  // Bab el-Mandeb collapsing simultaneously = red
  if (outbound && outbound < 30 && babOut && babOut < 5) return 'red';
  if (outbound && outbound < 30) return 'amber';
  // Inbound dropping faster than outbound = tankers pulling back
  if (outbound && inbound && inbound < outbound * 0.7) return 'amber';
  // Dark transits rising = sanctions evasion or risk avoidance increasing
